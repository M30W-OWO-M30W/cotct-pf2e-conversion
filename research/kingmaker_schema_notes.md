...
"reuse → adapt" mapping that drives the CotCT build scripts. Kingmaker content ©
Paizo; only schema/patterns are reused.
...
| **v13 `background` object + `foreground` roof + `thumb`**, grid px/distance/units | ✅ adopt | Scene `background.src` → expected Racooze map path; grid `type:1,size:<px>,distance:5,units:"ft"`. **No map shipped** → positions flagged `NEEDS GM REVIEW`. |
...
| Kingmaker **maps/art/tokens/audio/prose** | ❌ never | Copyright. Ship none; reference Racooze's free maps; token art = placeholders. |
