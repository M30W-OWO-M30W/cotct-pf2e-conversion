...
AW("blood-veil-plague-zombie", B.npc(A2["pzombie"], "Blood Veil Plague Zombie", 1, 14, 40, 7, 1, 4, 1,
  ...
  [B.strike(nid(), "Fist", 9, "1d6+4", "bludgeoning", []),
   B.action(nid(), "Blood Veil Taint", "passive", "<p>A creature damaged by the zombie's Strikes is exposed to <strong>blood veil</strong>: "+chk("type:fortitude|dc:18")+" or contract the disease (the staged affliction in the Conversion Guide). The cult's experimental strain rides in its fluids.</p>", category="offensive"),
   B.action(nid(), "Death Burst", "passive", "<p>When the zombie is destroyed, it ruptures in a spray of contagious fluids: each adjacent creature is exposed to blood veil ("+chk("type:fortitude|dc:18")+").</p>", category="offensive"),
...
PG("Yellow Lights", SR("Yellow Lights", 79)
  ...
  + B.s_skill("<p><strong>Asking around</strong> (Diplomacy to Gather Information) turns up rumors by result:</p>"
    ...
    "</tbody></table>"
    "<p>A "+chk("type:society|dc:20")+" (or Sailing Lore) identifies a single yellow bow-light as the Inner Sea signal for a ship <strong>under quarantine</strong>.</p>")
...
PG("The First Symptoms & A Second Favor", SR("The First Symptoms / A Second Favor", 80)
  ...
  + B.s_skill("<p><strong>Saving Brienna:</strong> she has <strong>Blood Veil</strong> ("+B.cond("drained", "Drained")+" from Con loss) and dies within days untreated. <em>Remove disease</em> cures it; <em>lesser restoration</em> staves off the drain. "+act(A2["ishani"], "Ishani")+" — out of spells today — suggests buying a <em>remove disease</em> potion from his temple, or for <strong>150 gp</strong> he returns to cast it next morning. "+chk("type:perception|dc:20")+" reads how ashamed he is to ask; "+chk("type:diplomacy|dc:12")+" persuades him to cast it <strong>free</strong> (he spends the rest of the day praying Abadar's forgiveness).</p>")
  ...
  + SEC("<p><strong>The tainted-coins clue.</strong> Cured, Brienna asks if lunch is ready — and goes evasive if pressed about how she got sick. She secretly fears Desna punished her for not sharing the coins; "+chk("type:diplomacy|dc:30")+" (or <em>detect thoughts</em>) coaxes the fear out, and the harder the PCs push, the more protective Tayce gets. The PCs aren't <em>meant</em> to get the coin lead this early — and the taint has already worn off the scattered silver, so scouring the city for coins now is a time-eating goose chase. Don't let it derail them; do let it pay off as foreshadowing when the death's head coffers surface (G14).</p>")
...
area2("B6", "Girrigz's Den", SR("B6", 94)
  ...
  + B.s_treasure("<p>"+chk("type:perception|dc:15")+": an arms cache (22 daggers, 12 short swords, 3 crossbows, 4 chain shirts, 12 alchemist's fire, a masterwork longsword + chainmail). "+chk("type:perception|dc:20")+" in the nest: 4 potions of healing, a bottle of air, and a <strong>pearl of power</strong> (2nd-rank) — plus the squeeze-crack to B3. Girrigz carries ~23 gp re-scaled besides his runed mithral rapier.</p>"))
...
area2("C1", "The Perfumery (Lavender)", SR("C1", 96)
  ...
  + B.s_treasure("<p>"+chk("type:society|dc:18")+" picks out genuine perfumes worth ~80 gp re-scaled; the counter box holds ~42 gp in recent sales, and the safe ("+chk("type:thievery|dc:28")+") ~76 gp in coin + three carved lapis perfume bottles (~5 gp each). Vendra carries a <strong>circlet of persuasion</strong>, a wand of <em>charm person</em>, a wand of <em>remove disease</em> (3 charges — she's saving the last to flee the city), and ~18 gp of silver-and-violet jewelry.</p>"))
...
area2("C2", "Vendra's Apartment", SR("C2", 99)
  ...
  + "<p>A locked ("+chk("type:thievery|dc:20")+"), prim parody of a Chelish noble's salon — tawdry romances, maudlin poetry, amateur art, and <strong>no alchemy anywhere</strong>. "+chk("type:perception|dc:20")+" finds a corner-hinged <strong>secret door</strong> into the boarded-up lab (C3). By night there's a 50% chance Vendra sleeps here; otherwise she's mixing brew in C3.</p>")
...
area2("D2", "The Workshop", SR("D2", 101)
  ...
  + "<p>Beneath the toy-shop: a 3½-foot dirt crawl space (difficult terrain for Medium creatures) under the trap door, where the four "+B.mon("vampire-servitor", "vampire spawn")+" keep their Ustalavic-soil coffins, watched over by the desiccated corpse of the toymaker Rodolfo Giotorri (eight neck-punctures; "+chk("type:perception|dc:15")+" finds his key ring). By day they sleep here (−10 to their Perception; killing one wakes the rest thrashing and howling). Fight all four here if the alley alarm was raised.</p>"
  ...
  + B.s_treasure("<p>"+chk("type:perception|dc:20")+" in the coffin soil: ~45 gp re-scaled of Ustalavic-minted coin, a <strong>ring of jumping</strong> (brass spider-legs), and <strong>pipes of haunting</strong> (bone).</p>"))
...
area2("D3", "Giotorri's Toys", SR("D3", 101)
  ...
  + "<p>The dusty, locked showroom above ("+chk("type:thievery|dc:30")+", or Giotorri's key from D2). The spawn left the cheap, defective toys alone — which is why nobody ever investigated.</p>"
  + B.s_treasure("<p>"+chk("type:perception|dc:14")+": a jingling lockbox ("+chk("type:thievery|dc:20")+") with a few coins, a silver tooth, and an <strong>Abadaran deposit-box key</strong> (No. 261; "+chk("type:society|dc:16")+" or "+chk("type:religion|dc:16")+" to place it). Redeemed at the Grand Vault, it yields Giotorri's life savings (~7 gp re-scaled), masterwork thieves' tools, <strong>boots of striding and springing</strong>, and a sheaf of terrible toy designs.</p>")
...
PG("What Jolistina Knows & Ruan's Fate", SR("What Jolistina Knows / Ruan's Fate", 109)
  + "<p>Captured alive, "+act(A2["jolistina"], "Jolistina")+" knows much — but she is thoroughly insane. Unless charmed or magically compelled, getting her to talk takes "+chk("type:diplomacy|dc:36")+". Whatever the result she giggles that the speaker's tone reminds her of 'my darling Rolth's silken tongue.'</p>"
  + B.s_skill("<p><strong>Intimidation is a trap with one lever:</strong> the forlorn elf is a fatalistic <strong>masochist</strong> — threats of pain, torture, even infection with blood veil delight her (expect exaggerated scandalous cries). She is immune to all of it <em>except</em> threats of <strong>imprisonment</strong> or of <strong>exposing her failure to Rolth</strong>: with that lever, "+chk("type:intimidation|dc:17")+" gets her talking.</p>")
...
area2("F3", "Loading Dock", SR("F3", 114)
  ...
  + B.s_treasure("<p>A thorough half-hour search and "+chk("type:perception|dc:20")+": a crate of elegant Vudrani silks — robes, saris, stoles, turbans — worth ~30 gp re-scaled. One sari is <strong>bloodstained and slashed</strong>: quiet evidence that not everything that happens in this building is medicine.</p>"))
...
area2("F4", "Warehouse Lift", SR("F4", 114)
  ...
  + "<p>The Red Mantis chose this warehouse <strong>for this room</strong>: the cargo lift is the cleverly concealed — and <strong>only</strong> — access to the hidden level below. Its controls plainly run the lift up to the floors above, but the operating <strong>lever radiates moderate necromancy</strong> (a tell for any caster who looks). "+chk("type:perception|dc:20")+" finds a second control set into the lever's slot: a simple socket <strong>missing its button</strong>. With the button-key seated and pressed, the lift descends to G1 in the Temple of Urgathoa; "+chk("type:thievery|dc:35")+" can force the mechanism to lower without it. <strong>The missing button is carried by Dr. Davaulus</strong> (F8).</p>"
...
area2("F6", "Private Hall", SR("F6", 115)
  ...
  + "<p>The old clerks' entry, now a guardroom. The doors to F7 are kept <strong>locked</strong> ("+chk("type:thievery|dc:30")+" or "+chk("type:athletics|dc:26")+"; Davaulus holds the key and personally opens them at shift change, dawn and dusk).</p>"
...
area2("F8", "The Doctor's Office", SR("F8", 116)
  ...
  + B.s_treasure("<p>"+chk("type:perception|dc:16")+" while searching the desk: <strong>Davaulus's scattered notes</strong> on the Varisians' immunity — one of the <strong>three note-sets</strong> behind the cure (→ Chapter Conclusion: Curing Blood Veil); recovering them is worth <strong>2,400 XP</strong>. His gear includes ~58 gp re-scaled, the <strong>lift button-key</strong>, the G3 cabinet and F7 door keys — and a <strong>flask of curses</strong> neatly labeled '<em>elixir of true healing</em>', left where a looter would grab it first ("+chk("type:will|dc:22")+" for the drinker).</p>")
...
area2("G6", "Patient Cells", SR("G6", 122)
  ...
  + "<p>Eight cramped cells behind iron doors (Hardness 10, HP 60, "+chk("type:athletics|dc:28")+" / "+chk("type:thievery|dc:30")+" — or any cultist's keys). <strong>Five hold captive Varisians</strong> of all ages and walks, taken within the last week; many watched earlier prisoners hauled off to the experiments and not return. All are blood-veil immune — the trait that doomed them — and can tell the PCs the same things as Olena and Dalvun (G5).</p>"
...
area2("G8", "The Blood Vats", SR("G8", 122)
  ...
  + B.s_skill("<p><strong>Destroying the vats:</strong> smash one (Hardness 10, HP 90) and its contents flood the lower floor — everyone standing there risks exposure, but the batch is ruined (inert after 1 hour). <em>Purify food and drink</em> works but slowly (a vat is ~133 cubic feet — about 2 minutes of repeated castings); a single <em>remove disease</em> cast <strong>on a vat</strong> (a counteract attempt against the brew's infection DC, above) neutralizes it outright. Reward clever alternatives.</p>")
...
area2("G12", "Arkminos's Room", SR("G12", 127)
  ...
  + "<p>The sleepless nosferatu's leisure: ancient healing texts, legendary accounts of vampires, religious lore. One trunk is <strong>locked</strong> ("+chk("type:thievery|dc:30")+"; forcing it with "+chk("type:athletics|dc:22")+" destroys the contents). "+chk("type:perception|dc:12")+" reveals the 'bed' is a frame stretched with black satin over one of <strong>Arkminos's traveling coffins</strong> — and it is here he flees if reduced to 0 HP (a fact that matters enormously to anyone intent on truly destroying him).</p>"
...
area2("G14", "The Inner Sanctum", SR("G14", 129)
  ...
  + "<p><strong>The seven scourges.</strong> The basins hold the seven fluids said to leak eternally from the Pallid Princess's necrotic body — bile, blood, milk, phlegm, pus, sweat, and tears — spontaneously generated foci of her worship, and here each has been used as a <strong>medium for blood veil</strong>. Touching any of the fluids forces a "+chk("type:fortitude|dc:26")+"; a creature that fails contracts blood veil with <strong>no incubation</strong>, taking the first stage's damage immediately. The empty skull-boxes are <strong>two dozen spent <em>death's head coffers</em></strong> — each once held some contaminant (diseased rats, blood, plague-victim flesh); the one that carried <strong>Vorel's phage</strong> became blood veil itself. <strong>One unused coffer</strong> remains among them — the item Ramoska covets (G11).</p>"
