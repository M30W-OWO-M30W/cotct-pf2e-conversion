...
# =====================================================================
# PREPARED SCENES — Racooze battlemap geometry + the keying fleet's pins and
# day-default token staging (research/scene_keys.json, chapter "ch4"). Pins
# link this journal's area pages; every token ships hidden (the GM reveals).
# =====================================================================
import html as _html
import json as _json

_scg = B._idgen(777004)            # scene-embedded ids: own deterministic stream so
def scid(): return next(_scg)      # no existing id shifts (884004 is taken by sids;
                                   # 777004 mirrors the pilot's 777001 scene stream)
SC_FOLDER = "ch4SceneFolder01"     # scene folder doc created in build_pilot's pass
MM = "icons/svg/mystery-man.svg"
_SZ = {"tiny": 1, "sml": 1, "med": 1, "lg": 2, "huge": 3, "grg": 4}
# Mother of Thorns: the keying brief staged her medium (so does the id map), but
...
with open("/tmp/actor_id_map.json", encoding="utf-8") as _f:
    _AMAP = _json.load(_f)
_PAGE_ID = {_html.unescape(p["name"]): p["_id"] for p in pages}
def _pgid(name):
    n = _html.unescape(name)
    if n not in _PAGE_ID:   # fail loudly — a silent mislink ships a broken pin
        raise KeyError(f"scene pin pageName not found in Ch.4 journal: {name!r}")
    return _PAGE_ID[n]

_SCN = {  # racooze scene name -> (pack write slug, sidebar navName)
  "The Acropolis of the Thrallkeepers": ("04-acropolis-of-the-thrallkeepers", "Acropolis"),
  "House of the Moon": ("04-house-of-the-moon", "House of the Moon"),
  "Cindermaw's Hunting Grounds": ("04-cindermaws-hunting-grounds", "Cindermaw"),
  "Flameford": ("04-flameford", "Flameford"),
  "Bolt Rock": ("04-bolt-rock", "Bolt Rock"),
  "Deathhead Vault": ("04-deathhead-vault", "Deathhead Vault"),
}
for _i, _sk in enumerate(_KEYS, 1):
    _rn = _sk["scene"]; _slug, _nav = _SCN[_rn]
    _ox, _oy = B.scene_origin(_rn)   # map-local grid squares -> scene px
    _notes = [B.note(scid(), JID4, _pgid(n["pageName"]), n["label"],
                     int(_ox + n["gx"] * 100), int(_oy + n["gy"] * 100))
              for n in _sk["notes"]]
    _toks = []
    ...
    _sc = B.racooze_scene(_rn, _sk["displayName"], SC_FOLDER, _notes, _toks,
                          navName=_nav, sort=_i * 100000)
    B.write("scenes", _slug, copy.deepcopy(_sc))

# --- "Prepared Scenes" journal page: scene links + GM staging digest of the
# keying pass (canvas layout, day-vs-night moves, hidden-finale spawns, and
# the keyers' honest ambiguity flags — compressed, nothing dropped).
def scn(rname, label):
    return f"@UUID[Compendium.{MODID}.cotct-scenes.Scene.{B.scene_id(rname)}]{{{label}}}"

PG("Prepared Scenes",
  "<p>Six prepared scenes ship with this chapter on Racooze's battlemaps: every keyed area pinned to its journal page, and the day-default occupants staged as <strong>hidden</strong> tokens — reveal them as the table earns them. The staging notes below digest the map-keying pass, including its judgment calls.</p>"

  + "<h4>" + scn("The Acropolis of the Thrallkeepers", "The Acropolis of the Thrallkeepers") + "</h4>"
  + "<ul>"
  + "<li><strong>Canvas:</strong> one 44×22 background holds <strong>both dungeon levels side by side</strong> — upper level (A1–A8) on the left (local x 0–28), lower level (A9–A11) on the right (x 28.5–44) across black void. The glowing A5 disc <em>feather-falls</em> 70 ft to the matching blue disc in A9's circular south chamber (~31,18 local); levitate returns from the lower ring.</li>"
  + "<li><strong>A2</strong> is a balcony ring around a 15-ft pool whose floor is the sleeping havero's flank; the pin sits on the mid-pool bridge. The 4 <strong>Havero Tentacle</strong> tokens (2×2) wait in the dark water and animate only at Noise Point thresholds (10 twitch · 20 one tentacle investigates · ~30 seeks · 40 two assault · 50 all four wrath); each slain tentacle −1d20 noise, and noise <em>doubles</em> in A2.</li>"
  + "<li><strong>Krojun + 3 Thundercallers</strong> (neutral — they arrived an hour early to take the party's measure, and aid against the havero) hide in the <strong>north A3 gallery</strong>; the text never says which gallery — relocate to the south corridor (~12,14 local) if preferred.</li>"
  + "<li>The 4 <strong>Red Mantis Assassins</strong> at the A1 stairs are a GM convenience: they actually enter ~10 minutes <em>behind</em> the party — keep hidden and walk them in when they strike (once discovered, or as the PCs leave).</li>"
  + "<li><strong>A8</strong>'s pin marks the north rubble end of the east service corridor (Thrallkeepers' quarters); the matching collapsed south branch (slave pens) sits at ~(23,20) local.</li>"
  + "<li>Room IDs rest on signature map features: A4 domed alcove relief, A5 rune-ringed disc, A6 bronze statue, A7 six-walled bone-strewn room, A10's 120-ton plug boulder, A11's floating globe.</li>"
  + "<li><strong>Not staged:</strong> Cinnabar (guides the hit team but waits outside; her lair is Deathhead E5), Koriantu (scries only, never present), and the havero itself (no actor exists — it <em>is</em> the pool floor; the tentacles represent it).</li>"
  + "</ul>"

  + "<h4>" + scn("House of the Moon", "House of the Moon") + "</h4>"
  + "<ul>"
  ...
  + "<h4>" + scn("Cindermaw's Hunting Grounds", "Cindermaw's Hunting Grounds") + "</h4>"
  + "<ul>"
  + "<li><strong>Canvas:</strong> 40×48 wilderness with no keyed codes — a single pin (C) on the elongated western cindercone outcrop, per the hunt directions. Terrain read: dark cracked ground = cinderlands, pale = ash flats, dark winding bands = gully/lava channels (the staging avoids them).</li>"
  + "<li><strong>Cindermaw</strong> (gargantuan, 4×4) is staged surfaced on the ash flat just east of the outcrop — it roams and hunts loiterers, so reposition freely and foreshadow its spark-gout surfacings before any direct encounter.</li>"
  + "<li><strong>Intent:</strong> the goal is the Skurak deed — one PC swallowed and out again before Shoanti witnesses — <em>not</em> a kill: +2 RP for the deed, +4 more if the worm lives, and slaying it costs RP. Severe 120 XP @ L11 if fought straight.</li>"
  + "</ul>"

  + "<h4>" + scn("Flameford", "Flameford") + "</h4>"
  + "<ul>"
  + "<li><strong>Canvas:</strong> 40×27 outdoor camp — tribe west-center, river gorge along the west edge, octagonal horse corral (D3, gate ~25.5,7.7 local) center-east, spike-stone approach belt east. D1's pin sits where the winding dirt trails converge: those visible trails <em>are</em> the unmarked safe zigzag paths (off-path: "+chk("type:perception|dc:31")+" per square).</li>"
  + "<li><strong>Day staging is the peaceful camp:</strong> Krojun (neutral — once the Trial is granted he storms off on a long ride and watches day two from the rocks) plus 6 Thundercallers spread through camp (fire ring, corral gate, chieftain's yurt, west camp, east lookout). No actors exist for Ready-Klar, the Sun Shaman, the ~38 warriors, the 8 burn riders, or the corral horses.</li>"
  + "<li><strong>Yurt IDs:</strong> D5 = the map's largest yurt (dark brown, SE by the corral — high confidence; its NE quarter is Ready-Klar's). D4 = the second-largest tan yurt north; D6 = the red-striped tent SW — both are size/visual judgment calls (the text gives no compass), and the unique blue yurt at (20.8,13.4) is the leading alternative for either. D2 = the stone fire ring (~10–13, 12–15), where the whole camp gathers at night.</li>"
  + "<li><strong>D* = the 20-RP dusk Assault drop zone</strong> among the yurts — an event, not default occupants. On trigger stage: 3–6 Elite Red Mantis Assassins air-dropped by 3–6 Ashwing Gargoyles over the spike fields; the Cinderlander dueling Krojun at the camp edge (~100 HP vs Krojun's ~30) with Neverfar (2×2) harrying at his side; Cinnabar joins a doubled assault only if she escaped the vault with a grudge.</li>"
  + "</ul>"

  + "<h4>" + scn("Bolt Rock", "Bolt Rock — Trial of the Totem") + "</h4>"
  + "<ul>"
  + "<li><strong>Canvas:</strong> 17×13 — two grass-topped plateaus over a dark chasm; no official codes, so BR1/BR2 are invented pins, both linking the Quest D page. <strong>Height call:</strong> west plateau = the 30-ft tier (day one), east = the 40-ft mesa (day two), read off the doubled cliff rims — swap freely.</li>"
  + "<li>The 6 hidden <strong>Bulettes</strong> (huge, 3×3, non-overlapping) are day two's Hungry Visitors (Moderate 90 XP @ L11): they burrow in with 2d4 rounds of warning and leap onto the <em>lower</em> tier — hence all staged on the west plateau.</li>"
  + "<li>The fired-clay <strong>sun totems</strong> (10 ft, 500 lbs, Hardness 6, HP 40) are props, not actors — drop 1–3 tile/drawing markers near BR1 on day one and BR2 on day two; a bulette passing one forces "+chk("type:athletics|dc:24")+" or it topples.</li>"
  + "<li>Otherwise empty endurance-trial terrain (heat, thirst, and fatigue per the quest page). The Sun Shaman (he only inspects for cheating) has no actor.</li>"
  + "</ul>"

  + "<h4>" + scn("Deathhead Vault", "Deathhead Vault") + "</h4>"
  + "<ul>"
  + "<li><strong>Canvas:</strong> single-level 60×40, three zones west→east: brick sewer + natural caverns (E1–E4a), Red Mantis sanctum + Shoanti crypts (E5–E9), and the worked Gray Maiden dungeon (E10–E21). The map's compass is <strong>mirrored</strong> against several book directions — every room was identified by its furniture instead, which is unambiguous (three vats + cot = E5; six pillars + mantis statue = E6; dais sarcophagus ringed by vinegar vats = E8; ivory tiles + executioner's block = E11).</li>"
  + "<li><strong>West run:</strong> E1 sewer (Snagglebreath in the trough) → E2 tavern → E3 cavern, whose plank bridge crosses the 60-ft spike pit to the NE ledge — the hidden <strong>Red Mantis watcher</strong> waits beyond it; cytillesh brain-fungus fields run south from E3 into E4. E4a's pin = the rubble-choked camp NW of E4 (moderate confidence — it is the only blocked north passage on the map).</li>"
  + "<li><strong>E4, the crescent cathedral:</strong> nine bunk alcoves scallop its S/W rim <em>40 ft up</em> ("+chk("type:athletics|dc:26")+") — the two resting assassins occupy two of them. The pulpit stairs climb east to the mantis-facade double doors, which on this map open <em>directly into E6</em>; E5 lies deeper still (E6's illusion-hidden north door → E7 → E8, with E5 off E8's SW corner).</li>"
  + "<li><strong>Day-pose ruses staged hostile for GM clarity:</strong> Mogmora (2×2) plays friendly goblin barkeep behind E2's bar (dimension-doors to E6 to report after 1d4 rounds; shrink her to 1×1 for the disguise if desired — her footprint overlaps the bar) and the two manananggals wear human faces atop E3's ledges, crying out as escaped prisoners. Flip them to neutral if the canvas should keep the secret.</li>"
  + "<li><strong>Crypts:</strong> E7's six sarcophagus alcoves are looted derro cages (empty — nothing staged); E9 = the Society's secret dog-leg stair linking E10's south end to E6's south arm (secret doors at both ends; Boule's +15 applies); E10 = the bright ivory L-corridor to E11's doors.</li>"
  + "<li><strong>Maiden-wing IDs:</strong> E12 = the office north of E11 sharing the windowed-iron-door wall (its dark boulder reads as the commanders' secret-tunnel door, "+chk("type:perception|dc:33")+"). E14 = the isolated chained cell with no visible door (Marcus Endrin staged, friendly). E15 = the barred cage + hall (2 guards firing through the bars with cover — one shouts the alarm — and 2 in the hall; NW stairs climb to the Longacre ground floor). E16 = eight identical bedrooms with 4 resting guards (~5 minutes to armor up on an alarm). E18 = the torture chamber: Zenobia mid-indoctrination by the south cage, 3 caged recruits, 5 guards at the west watch post. E17 = Tisharue's study-bedroom; E21 = Kordaitra at her desk over Boule's fake ciphers. <strong>E19/E20 were assigned by elimination — moderate confidence, review.</strong></li>"
  + "<li>E20's thorny bramble mass is almost certainly map-art for the <strong>Mother of Thorns</strong>' nest, but the text stations her in E11 — she is staged there, west of the block; at ~40 HP she teleports to E18 and heals at Zenobia's side.</li>"
  + "<li><strong>E13:</strong> the 12 imprisoned women sit one per perimeter cell (4 north, 5 east, 3 south) — beaten to 1d4 HP, neutral, in no condition to aid. 15 freeable women total with E18's caged recruits.</li>"
  + "<li><strong>The alarm cascade is NOT pre-staged</strong> (day defaults are): once roused, everything that flees converges on E4 with Cinnabar + Koriantu behind them (Extreme 175 XP), and Kordaitra plus the Mother of Thorns/Vyloth fallbacks stack E18 to Extreme 190 XP. Thin the pile before it stacks.</li>"
  + "<li><strong>Sizes:</strong> Snagglebreath (catoblepas), Cytillipede, Mogmora, and Vyloth ship 2×2; the Mother of Thorns also ships 2×2 (her actor is Large, though the keying brief listed her medium); everyone else is 1×1.</li>"
  + "</ul>")
...
print(f"Chapter 4 built: {len(folders)} folders, {len(actors)} actors, {len(hazards)} hazards, "
      f"{len(_KEYS)} prepared scenes, 1 journal ({len(pages)} pages).")
